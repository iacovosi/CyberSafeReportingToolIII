...
<div class="container">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-heading clearfix">
            <h4 class="pull-left"><i class="fa fa-cogs" aria-hidden="true"></i> Reference to</h4>
            <div class="pull-right form-actions">
                @if(GroupPermission::usercan('create','content'))
                    <a href="/referenceTo/create" class="btn btn-primary">
        ...
        <div class="panel-body">
            ...
            <div class="table-responsive">
                <table class="table table-striped table-hover">
                    ...
                    <tbody>
                        @if ($referenceTo) 
                        @foreach ($referenceTo as $key=>$item)
                            <tr>
                                <td>{{ ++$key }}</td>
                                {{--  <td>{{$item->id}}</td>  --}}
                                <td>{{$item->name}}</td>
                                ...
                                <td style='white-space: nowrap'>
                                    ...
                                    <form method="POST" action="/referenceTo/{{$item->id}}">
                                        {{--  Other actions are included in the form for styling purposes  --}}
                                        <a href="{{route('referenceTo.show', $item->id)}}" class="btn btn-default">
                                            <i class="fa fa-eye" aria-hidden="true"></i> Show
                                        </a>
                                        <a href="{{route('referenceTo.edit', $item->id)}}" class="btn btn-default">
