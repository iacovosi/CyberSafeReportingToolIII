...
<div class="container-fluid">
    ...
    <p>@lang('translations.formHotline.intro')</p>

    <form method="post" action=" {{ route('save-helpline') }}" class="userReportForm hotlineForm">
        ...
        <fieldset class="scheduler-border">
            ...
            <div class="form-group">
                ...
                @foreach($content_types as $content_type)
                    @if ($content_type->is_for == "hotline" && $content_type->name != "irrelevant" )                                        
                        <div class="radio">
                            <label data-toggle="tooltip" data-placement="top">
                                <input type="radio" name="content_type" value="{{ $content_type->name }}" @if (old('content_type') == $content_type->name) checked  @endif>
                                {{ $content_type->display_name_gr }} <i class="fa fa-info-circle" aria-hidden="true"  data-toggle="popover"  data-trigger="hover" data-placement="bottom" data-content="{{ $content_type->description_gr }}"></i>
                            </label>
                        </div>
                    @endif
                @endforeach
        ...
        <fieldset class="scheduler-border">
            ...
            <div class="form-group">
                ...
                <div class="help-block">Μέγιστος αριθμός χαρακτήρων 2500 <span class="notification"></span></div>
            </div>
        </fieldset>
                        ...
                        <div class="form-group">
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                ...
                                <label class="btn btn-default  @if (old('age') == $age_group->name) active  @endif">
                                    <input type="radio" name="age" value="{{$age_group->name}}" @if (old('age') == $age_group->name) checked  @endif >{{$age_group->display_name_gr}}
                                </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <div class="btn-group" data-toggle="buttons">
                                ...
                                <label class="btn btn-default @if (old('gender') == $gender->name) active  @endif">
                                    <input type="radio" name="gender" value="{{$gender->name}}" @if (old('gender') == $gender->name) checked  @endif>{{$gender->display_name_gr}}
                                </label>
                                @endforeach
                            </div>
                        </div>
...
<script>
    $(document).ready(function () {
        ...
        $('textarea#comments').on('keyup',function(){
            ...
            if (charLeft <= 2000) {
                $(".notification").text(" (απομένουν " + charLeft + ")");
            }
        });
	});
    ...
    setInterval(function() {
        window.top.postMessage(document.body.scrollHeight, "https://www.cybersafety.cy/hotline-report");
    }, 500);
</script>
@endsection
